...
namespace Lesson7_封装_静态成员
{
    ...
    class Test
    {
        ...
        public static float CalcCircle(float r)
        {
            ...
            #region 知识点六 静态函数不可以使用静态成员
            //成员变量只能将对象实例化出来以后 才能点出来使用，不能无中生有
            //不能直接使用 非静态成员 否则会报错
            //testInt = 10;
            Test t = new Test();
    ...
    //之所以要实例化对象，目的就是为了分配内存空间，在程序中产生一个抽象的对象

    //静态成员的特点
    //程序开始运行使， 就会分配内存空间，所以我们能直接使用
    //静态成员和程序同生共死
    //只要使用了它，直到程序结束时内存空间才会被释放
    //所以一个静态成员就会有自己唯一的一个”内存空间“
    //这让静态成员就有了唯一性
    ...
    //3.const一定是写在访问修饰符后面的，static 没有这个要求
    #endregion
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("Lesson7_封装_静态成员");
            #region 知识点四 静态成员的使用
            Console.WriteLine(Test.PI);
            Console.WriteLine(Test.CalcCircle(3));

...
{
    #region 知识点一 特性是什么
    //特性是一种允许我们向程序的程序集添加元数据的结构
    //它是用于保存程序结构信息的某种特殊类型的类

    //特性提供功能强大的方法以声明信息与C#代码（类型、方法、属性等）相关联
    ...
    //特性本质就是个类
    //我们可以利用特性类为元数据 添加额外信息
    //比如一个类、成员变量、成员方法等等为它们添加更多的的额外信息
    //之后可以通过反射来获取这些额外信心
    #endregion
    ...
    //本质上 就是在调用特性类的构造函数
    //写在哪里？
    //类、变量、函数上一行，表示它们具有该特性信息
    [MyCustom("这个是我自己写的用于计算的类")]
    class MyClass
    {
        [MyCustom("自定义值")]
        public int value;
        ...
        [MyCustom("这是用于加法的函数")]
        public void TestFun([MyCustom("需要输入")]int value)
        {
    ...
    {
        //参数一:调用过时方法 提示的内容
        //参数二：true:使用该方法时会报错  false--会直接警告
        [Obsolete("来自周某的提示：OldSpeak方法已经过时了，请用speak方法", false)]
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (obj[i] is MyCustomAttribute)
                {
                    Console.WriteLine((obj[i] as MyCustomAttribute).info);
                    (obj[i] as MyCustomAttribute).TestFun();
                }
            }

            TestClass tc = new TestClass();
            tc.OldSpeak("123");
            tc.SpeakCaller("1234564");
